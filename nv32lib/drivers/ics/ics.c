...
void FEI_to_FEE(ICS_ConfigType *pConfig)
{
  ...
  /*对选中的ICS输出时钟源做1分频*/
	...
	{
		ICS->C2 = (ICS->C2 & ~(ICS_C2_BDIV_MASK)) | ICS_C2_BDIV(BDIV_ENCODE);
	}
#else
    ICS->C2 = (ICS->C2 & ~(ICS_C2_BDIV_MASK)) | ICS_C2_BDIV(BDIV_ENCODE);
#endif
...
void ICS_SetClkDivider(uint32_t u32ClkFreqKHz)
{
    ...
    switch(u32ClkFreqKHz)
    {
        ...
        case 10000L:
            /* 8MHz or 10MHz*/
            ICS->C1 = (ICS->C1 & ~(ICS_C1_RDIV_MASK)) | ICS_C1_RDIV(3);	 /*8MHz分频结果是 8000/256 = 31.25K */       
																																				 /*10MHz分频结果是 10000/256 = 39.0625K*/ 
            break;
        case 4000L:
            /* 4MHz */
